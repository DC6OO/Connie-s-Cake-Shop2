(function () {
  ...
  if (productImageEl) {
    productImageEl.addEventListener("change", function () {
      ...
      if (!file) return;
      showStatus(productStatus, "Processing photo…");
      window.ShopData.fileToDataUrl(file)
        .then(function (dataUrl) {
          pendingProductImage = dataUrl;
          if (productPreviewImg) productPreviewImg.src = dataUrl;
          if (productPreview) productPreview.hidden = false;
          showStatus(productStatus, "Photo ready.");
        })
        .catch(function (err) {
          showStatus(productStatus, err.message || "Could not use that photo.");
        });
    });
  }
  ...
  if (galleryImageEl) {
    galleryImageEl.addEventListener("change", function () {
      ...
      if (!file) return;
      showStatus(galleryStatus, "Processing photo…");
      window.ShopData.fileToDataUrl(file, 1200, 0.78)
        .then(function (dataUrl) {
          pendingGalleryImage = dataUrl;
          if (galleryPreviewImg) galleryPreviewImg.src = dataUrl;
          if (galleryPreview) galleryPreview.hidden = false;
          showStatus(galleryStatus, "Photo ready.");
        })
        .catch(function (err) {
          showStatus(galleryStatus, err.message || "Could not use that photo.");
        });
    });
  }
  ...
  if (slideImageEl) {
    slideImageEl.addEventListener("change", function () {
      ...
      if (!file) return;
      showStatus(slideStatus, "Processing photo…");
      window.ShopData.fileToDataUrl(file, 1600, 0.8)
        .then(function (dataUrl) {
          pendingSlideImage = dataUrl;
          if (slidePreviewImg) slidePreviewImg.src = dataUrl;
          if (slidePreview) slidePreview.hidden = false;
          showStatus(slideStatus, "Photo ready.");
        })
        .catch(function (err) {
          showStatus(slideStatus, err.message || "Could not use that photo.");
        });
    });
  }

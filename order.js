(function () {
  ...
  function renderPriceList() {
    var cardsHost = document.querySelector("[data-product-cards]");
    if (!cardsHost) return;
    cardsHost.innerHTML = "";
    PASTRY_ITEMS.forEach(function (p) {
      ...
      var media = p.image
        ? '<div class="product-card-media"><img src="' +
          p.image +
          '" alt="' +
          escapeHtml(p.name) +
          '" /></div>'
        : '<div class="product-card-media product-card-media-empty"><span>No photo</span></div>';
      card.innerHTML =
        media +
        '<div class="product-card-body"><h3>' +
        escapeHtml(p.name) +
        "</h3><p>" +
        formatMoney(p.price) +
        "</p></div>";
